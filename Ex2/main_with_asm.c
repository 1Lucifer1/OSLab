        ...
        if (l_param) {
            ...
            while (listTotal[pos].type != -1) {
                if (listTotal[pos].type == FILE_TYPE && listTotal[pos].startPos != 0) {
                    ...
                    my_print(listTotal[pos].name, strlen(listTotal[pos].name));
                    prepareOutputFile();
                    printf("  %d %d\n", listTotal[pos].dirNum, listTotal[pos].fileNum);
                } else {
                    prepareOutputDirectory();
                    my_print(listTotal[pos].name, strlen(listTotal[pos].name));
                    prepareOutputFile();
            ...
            printf(":\n");
            while (listTotal[pos].type != -1) {
                if(listTotal[pos].type == DIR_TYPE){
                    prepareOutputDirectory();
                    my_print(listTotal[pos].name, strlen(listTotal[pos].name));
                    my_print("  ", 2);
                    prepareOutputFile();
                }else if(listTotal[pos].name[0] == '.'){
                    prepareOutputDirectory();
                    my_print(listTotal[pos].name, strlen(listTotal[pos].name));
                    my_print("  ", 2);
                    prepareOutputFile();
